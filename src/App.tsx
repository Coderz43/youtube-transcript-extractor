...
function MainLayout() {
  ...
  const displayedTranscript = search && transcript
    ? transcript.filter(line => line.toLowerCase().includes(search.toLowerCase()))
    : transcript;
  ...
  // Fetch transcript from Flask API
  const transcriptData = await fetchTranscript(videoId);

  // Handle Flask error response
  if (transcriptData && typeof transcriptData === "object" && "error" in transcriptData) {
  ...
  <div className={`mt-6 ${
    ...
    <div className="flex items-center justify-between mb-4">
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="flex items-center gap-2 px-2.5 py-1 bg-[#ff4571] text-white rounded-lg hover:bg-[#ff3561] transition-colors text-sm font-medium"
    ...
    <div className={`max-h-[400px] overflow-y-auto rounded-lg ${
      ...
      {search && transcript.filter(line =>
        line.toLowerCase().includes(search.toLowerCase())
      ).length === 0 && (
        <div className="text-gray-400 py-4 text-center">No matches found.</div>
      )}
    </div>
  </div>
)}
